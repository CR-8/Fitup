...
Fitup's workout core uses SQLite on the device. SyncLayer is enabled only when `EXPO_PUBLIC_SYNC_HOST` contains a provider URL.
...
- The app does not pull the `fitup` exercise catalogue.
...
The maintained Fitup catalogue is currently delivered through the provider's separate `fitup` scope. It is not bundled with this repository. A new local-only installation therefore starts without the system catalogue.
