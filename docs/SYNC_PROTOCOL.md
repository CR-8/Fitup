...
Fitup's mobile client is local-first. This document describes the optional provider contract used when a build sets `EXPO_PUBLIC_SYNC_HOST`.

The Fitup-operated SyncLayer is one implementation. Another server can implement the same contract and serve a custom build. Provider selection currently happens at build time.
...
| `type`    | `user`, `fitup`, or `all`                          |
| `locale`  | Exercise locale for the `fitup` scope              |
...
The `fitup` scope distributes maintained exercise records separately from user-created exercises. Current field definitions live in `src/db/schema/`.
