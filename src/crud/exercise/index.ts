...
export const isFitupExercise = (row: Pick<ExerciseSelect, 'userId'> | null | undefined): boolean =>
    isFitupExerciseUserId(row?.userId);

const exerciseScopeCondition = (userId: string) =>
    or(eq(exercise.userId, userId), eq(exercise.userId, FITUP_EXERCISES_USER_ID));

export interface ExerciseLibrarySnapshot {
    exerciseLibraryTotalCount: number;
    exerciseLibraryFitupCount: number;
    ...
    try {
        ...
        const [counts] = await db
            .select({
                ...
                exerciseLibraryFitupCount: sql<number>`coalesce(
                    sum(case when ${exercise.userId} = ${FITUP_EXERCISES_USER_ID} then 1 else 0 end),
                    0
                )`,
        ...
        return {
            ...
            exerciseLibraryFitupCount: Number(counts?.exerciseLibraryFitupCount ?? 0),
...
const buildForkedExerciseRecord = (
    ...
    return {
        ...
        userId,
        source: 'user',
        fitupSourceId: source.fitupSourceId ?? source.id,
        createdAt: now,
        updatedAt: now,
    };
};
...
export const updateExercise = async (
    ...
    try {
        ...
        if (isFitupExercise(existingExercise)) {
...
export const mergeExercise = async (
    ...
    try {
        ...
        if (!isFitupExercise(sourceExercise)) {
...
export const deleteExercise = async (id: string, userId: string): Promise<void> => {
    try {
        ...
        if (isFitupExercise(exerciseToDelete)) {
