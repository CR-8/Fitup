...
export const backfillSyncQueue = async (): Promise<void> => {
    ...
    // Exercises: only queue user-created exercises, not fitup library exercises
    const userExercises = await db
        .select()
        .from(exercise)
        .where(and(gt(exercise.updatedAt, lastSync), ne(exercise.userId, FITUP_EXERCISES_USER_ID)));
