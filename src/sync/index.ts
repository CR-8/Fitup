...
const FITUP_EXERCISE_DATASET_VERSION = 1;
const FITUP_EXERCISE_DATASET_VERSION_KEY_PREFIX = 'fitup.dataset.exercise.version';

const getFitupExerciseDatasetVersionKey = (locale: string) =>
    `${FITUP_EXERCISE_DATASET_VERSION_KEY_PREFIX}.${locale}`;

const getLocalFitupExerciseDatasetVersion = (locale: string): number => {
    const version = storage.getNumber(getFitupExerciseDatasetVersionKey(locale));
    return typeof version === 'number' && Number.isFinite(version) ? version : 0;
};

const markFitupExerciseDatasetVersionSynced = (locale: string) => {
    storage.set(getFitupExerciseDatasetVersionKey(locale), FITUP_EXERCISE_DATASET_VERSION);
};
...
const applyFitupSyncPacks = async (
    packs: Record<string, any>,
    options: { replaceExistingFitup: boolean },
) => {
    await db.transaction(async (tx) => {
        ...
        if (options.replaceExistingFitup) {
            await tx.delete(exercise).where(eq(exercise.userId, FITUP_EXERCISES_USER_ID));
        }
...
const pullFitupChanges = async (
    ...
    try {
        ...
        const shouldBackfillFitupExercises =
            options.full !== true &&
            getLocalFitupExerciseDatasetVersion(locale) < FITUP_EXERCISE_DATASET_VERSION;
        const previousSyncTime =
            options.full || shouldBackfillFitupExercises
                ? new Date(0)
                : await getFitupLastSyncTimestamp(locale);
        const since = options.full || shouldBackfillFitupExercises ? 0 : previousSyncTime.getTime();
        ...
        if (result.success && result.data) {
            await applyFitupSyncPacks(result.data, {
                replaceExistingFitup: options.full === true,
            });
            const maxTimestamp = resolveMaxPackTimestamp(result.data, since);
            await updateFitupLastSyncTimestamp(locale, new Date(maxTimestamp));
            markFitupExerciseDatasetVersionSynced(locale);
            return { success: true };
        }
        ...
        captureSyncFailure('pull_fitup_response', {
            since,
            userId: currentUser.id,
            locale,
            full: options.full === true,
            shouldBackfillFitupExercises,
            response: result,
        });
        return { success: false };
    } catch (error) {
        captureSyncFailure('pull_fitup_exception', { options }, error);
        return { success: false };
    }
};

export const performFitupSync = async (
    options: {
        locale?: string;
        full?: boolean;
    } = {},
    ...
    const result = await pullFitupChanges(options);
